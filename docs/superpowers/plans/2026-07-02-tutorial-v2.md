...
Replace the entire contents with the fixture below, then **extend it to ~1.5× the item counts** (Kate's request): add fictional items in the same voice and format so the totals become — Research Brief ×3, Report ×2, P&O ×2, ERC Events ×3, Happy Hours ×1, Texas A&M events ×3, Off-Campus & Online ×4, F&G ×3, F&T ×2, Calls ×3, Working Papers ×4, Peer Reviewed ×2, Miscellaneous ×3, Federal ×4, Texas ×4 (≈41 items). Every added item follows the same field pattern as its siblings, fully fictional, all URLs `https://example.com/...`, typographic punctuation in prose. Group headings `## …` must stay exactly as written — the builder matches them by name:
