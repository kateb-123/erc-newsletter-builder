...
- `npm test` baseline is **54 passing** (46 non-tutorial + 8 tutorial). After Task 2 the expected count is **55** (46 + 9 rewritten tutorial tests); it stays 55 for all later tasks.
...
Run: `npm test`
Expected: **55 passing, 0 fail** (46 non-tutorial + 9 tutorial). `node --check js/tutorial-core.js` clean.
...
Run: `npm test` → 55/55 (this file has no node tests; count must match Task 2's).
...
Run: `npm test` → 55/55 (the node suite parses its own fixtures, not this one — count unchanged; if any test DOES read `sample-real.md`, stop and report BLOCKED with the failure).
